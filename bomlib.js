...
var ac = 2; //0 small blocks <--> 10 big blocks
...
function logicalObject(obj) {
    ...
    this.addGeometricObject = function(geo) {
        ...
        var sfc;
        var bgc;
        bgc = window.getComputedStyle(geo.element,null).getPropertyValue("background-color");
        fc = parseInt(window.getComputedStyle(geo.element,null).getPropertyValue("font-size"));
        if (bgc!="rgba(0, 0, 0, 0)") {
        ...
        if (geo.element.parentNode) {
            ...
            for (var k=0;k<geo.element.parentNode.children.length;k++) {
                if ( (geo.element.parentNode.children[k]!=geo) ) {
                    var sfc = parseInt(window.getComputedStyle(geo.element.parentNode.children[k],null).getPropertyValue("font-size"));
                    if (2*sfc < fc)
                        t++;
                }
            }
